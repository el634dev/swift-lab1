/*:
# Type Inference
Swift has a language feature called _type inference_. That means that the compiler will try to automatically figure out the type of 
each variable on its own. However, it's often better to be explicit by declaring the type of a variable in the code. That way, both the 
compiler and other programmers reading the code can be sure of your intention. To excplicitly declare the type of a value 
(like a `String` or `Int`) in a variable, do the following:
 */
var population: Int = 20
/*:
 From now on, `population` will always be an integer (shortened as `Int`). An integer is a whole number - a number without decimal 
 points. Let's see what happens when we try to set `population` to a decimal number.
